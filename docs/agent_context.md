...
- **Security**: The MSIX is signed using a custom self-signed certificate (`ExploreDB.pfx`). Because it is self-signed, end users must run `trust_certificate.bat` one time to inject it into their Trusted Root Certification Authorities store.
